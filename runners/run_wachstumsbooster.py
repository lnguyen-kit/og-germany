...
def main():
    ...
    cluster = LocalCluster(processes=False, n_workers=1, threads_per_worker=num_workers)
    ...
    updated_params_reform = {
        "cit_rate" : [
            ...
            [0.27],  # Jahr 6 (2030)
            [0.26],  # Jahr 7 (2031)
            [0.25],  # Jahr 8 (ab 2032 -> anschließend konstant 0.25)
            ],
        "delta_tau_annual": [
            [0.08140],  # 2025
            [0.08250],  # 2026
    ...
    cluster2 = LocalCluster(processes=False, n_workers=1, threads_per_worker=num_workers)
    client2 = Client(cluster2)
    print("Dask Client (Reform) gestartet: ", client2.dashboard_link)

    # Calibration muss neu initialisiert werden mit dem neuen Client
    c2 = Calibration(p2, estimate_tax_functions=True, client=client2)

    # Run model
    print (">>>> running reform (cit only)")
    start_time = time.time()
    #runner(p2, time_path=True, client=client)
    runner(p2, time_path=True, client=client2)
